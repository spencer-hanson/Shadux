#Gets certificate data and installs it to the system
#!/bin/bash
function usage() {
	echo Usage $0 [--install] [--update]
	echo --install installs the certificates to the system
	echo --update updates the certificates data
}
...
if [ "$1" = "--update" ] ; then
	echo Updating CertAuth...
	cd openssl
	wget mxr.mozilla.org/mozilla/source/security/nss/lib/ckfw/builtins/certdata.txt?raw=1
	mv certdata.txt?raw=1 ../data/certdata
	echo Done!
	exit
fi
...
for tempfile in ${TEMPDIR}/certs/*.tmp; do
  ...
  cp "${tempfile}" tempfile.cer
v  perl ${CONVERTSCRIPT} > tempfile.crt
  keyhash=$(openssl x509 -noout -in tempfile.crt -hash)
